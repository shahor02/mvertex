...
bool MVertexFinder::FindNextVertex(std::vector<MVertexFinder::vtxTrack> &tracks,float zseed,float sigScale2Ini, float zmin,float zmax)
{
  ...
  while(nIter++<mMaxVtxIter) {
	...
	if (zLR[1]<zmax) FindNextVertex(tracks,zLR[1],scaleSigma2, vtx.mXYZ[2],zmax);  // right: curZ<ZR<zmax	
				 ...
				 float zLR[2]) const
{
  ...
  for (int is=2;is--;) zLR[is] = wghLR[is]>kAlmost0F ? zLR[is]/wghLR[is] : kHugeF;
